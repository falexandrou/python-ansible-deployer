...
def test_email_notifications_sent():
    assert 'notifications_output' in PROVISIONING_OUTPUT
    assert 'email' in PROVISIONING_OUTPUT['notifications_output']
    out = PROVISIONING_OUTPUT['notifications_output']['email']
    msg = 'Mail sent successfully'
    assert all((entry['msg'] == msg for entry in out))
