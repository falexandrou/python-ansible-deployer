...
def test_user_created():
    ...
    assert meta.get('user_name') == 'stackmatemailer-ezploy-eu'
    ...
    assert policy['user_name'] == 'stackmatemailer-ezploy-eu'
    assert policy['policies'] == ['iam-policy-ezploy-eu']


def test_ses_domain_created():
    assert 'ses_domain' in PROVISIONING_OUTPUT
    assert PROVISIONING_OUTPUT['ses_domain'].get('identity') == 'ezploy.eu'
...
def test_stackmate_user_actions_created():
    ...
    assert 'fotis@ezploy.eu' in action['value']
